...
export class SS6Player extends PIXI.Container {
  ...
  private Update(delta: number): void {
    ...
    if (toNextFrame && this.updateInterval !== 0) {
      ...
      let s = dt / (1.0 / this.curAnimation.fps());

      let next = this._currentFrame + s * this.playDirection; // currently force frame skip
      ...
      this.SetFrameAnimation(Math.floor(this._currentFrame), s);

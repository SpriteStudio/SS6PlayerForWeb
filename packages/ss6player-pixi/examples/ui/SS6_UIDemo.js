...
(async () => {
  ...
  bg.width = app.canvas.width;
  bg.height = app.canvas.height;
  bg.interactive = true;
  bg.interactiveChildren = true;
  bg.hitArea = new PIXI.Rectangle(0, 0, app.canvas.width, app.canvas.height);
  bg.zIndex = 0;
  ...
  for (let ssfbName in ssfbFiles) {
    mySS6Projects[ssfbName] = new ss6PlayerPixi.SS6Project(ssfbFiles[ssfbName], () => {
      bg.emit('loadedssfb', ssfbName, true);
    });
  }
  ...
  function LoadButtonAnimation(ss6project) {
    ...
    const item_x = 150;
    const item_y = 100;
    const MAX_ROW_NUM = 1 + Math.floor((app.canvas.width - (offset_x * 2)) / item_x);
    const column = Math.floor(offset_y + (animePackNames.length / MAX_ROW_NUM) * item_y);
    if (column > app.canvas.height) {
      console.log("Probably, lack of Pixi.js canvas height for showing all buttons. canvas height: " + app.canvas.height + " button column: " + column);
    }
  ...
  function LoadClickAnimation(clickSS6Project) {
    const clickEvent = (PIXI.isMobile.any) ? 'tap' : 'click';
  ...
  function PlayButtonAnimation(SS6Project, animePackName, position, scale) {
    const mobileAny = PIXI.isMobile.any;
    ...
    const inEvent = (mobileAny) ? 'touchstart' : 'mouseover';

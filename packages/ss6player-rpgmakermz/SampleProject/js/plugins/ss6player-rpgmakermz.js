/**
* -----------------------------------------------------------
* SS6Player For RPG Maker MZ v0.9.0
*
...
*
*/
(function(_pixi_loaders, _pixi_display, _pixi_mesh, _pixi_ticker, _pixi_filter_color_matrix, _pixi_constants) {
	//#region ../ss6player-pixi6/dist/ss6player-pixi6.es6.js
	/*!
	* -----------------------------------------------------------
	*  SS6Player For pixi.js v6 v1.0.6
	*
	*  Copyright(C) CRI Middleware Co., Ltd.
	*  https://www.webtech.co.jp/
	* -----------------------------------------------------------
	...
	*/
	...
	var PixiResourceLoaderImpl = class {
		constructor() {
			this.loader = new _pixi_loaders.Loader();
		}
		load(sspjfile, sspjMap, onComplete) {
			for (let key in sspjMap) this.loader.add(key, sspjMap[key]);
			const self = this;
	...
	*/
	...
	var SS6Player = class SS6Player extends _pixi_display.Container {
		get startFrame() {
		...
		*/
		constructor(ss6project, animePackName = null, animeName = null) {
			...
			this.alphaBlendType = [];
			this.defaultColorFilter = new _pixi_filter_color_matrix.ColorMatrixFilter();
			...
			if (animePackName !== null && animeName !== null) this.Setup(animePackName, animeName);
			this.on("added", (...args) => {
				_pixi_ticker.Ticker.shared.add(this.Update, this);
			}, this);
			this.on("removed", (...args) => {
				_pixi_ticker.Ticker.shared.remove(this.Update, this);
			}, this);
		}
		/**
		* Setup
		...
		*/
		UpdateInternal(delta, rewindAfterReachingEndFrame = true) {
			const elapsedTime = _pixi_ticker.Ticker.shared.elapsedMS;
		...
		GetPartsBlendMode() {
			...
			for (let i = 0; i < l; i++) {
				...
				switch (alphaBlendType) {
					case 0:
						blendMode = _pixi_constants.BLEND_MODES.NORMAL;
						break;
					case 1:
						blendMode = _pixi_constants.BLEND_MODES.MULTIPLY;
						break;
					case 2:
						blendMode = _pixi_constants.BLEND_MODES.ADD;
						break;
					case 3:
						blendMode = _pixi_constants.BLEND_MODES.NORMAL;
						break;
					case 4:
						...
						break;
					case 5:
						blendMode = _pixi_constants.BLEND_MODES.SCREEN;
						break;
					case 6:
						blendMode = _pixi_constants.BLEND_MODES.EXCLUSION;
						break;
					case 7:
						blendMode = _pixi_constants.BLEND_MODES.NORMAL;
						break;
		...
		*/
		GetColorMatrixFilter(blendType, rate, argb32) {
			...
			const colorMatrix = new _pixi_filter_color_matrix.ColorMatrixFilter();
			...
			for (let ii = 0; ii < l; ii = ii + 1 | 0) {
				...
				switch (partType) {
					...
					case SsPartType.Joint:
						if (this.prevCellID[i] !== cellID) {
							if (partObject != null) partObject.destroy();
							partObject = new _pixi_display.Container();
							partObject.name = part.name();
						}
						break;
					default:
				...
				switch (partType) {
					...
					case SsPartType.Mask: {
						...
						let verts;
						if (partType === SsPartType.Mesh) if (data.meshIsBind === 0) verts = this.playerLib.TransformMeshVertsLocal(Player.GetMeshVerts(cell, data, meshVertex), data.index, frameNumber);
						else verts = Player.GetMeshVerts(cell, data, meshVertex);
						else {
						...
						if (blendMode === _pixi_constants.BLEND_MODES.MULTIPLY || blendMode === _pixi_constants.BLEND_MODES.SCREEN) mesh.alpha = 1;
		...
		*/
		MakeCellMesh(id) {
			...
			return new _pixi_mesh.Mesh(new _pixi_mesh.MeshGeometry(new Float32Array([
				0,
				0,
				-w,
				-h,
				...
				1,
				3
			])), new _pixi_mesh.MeshMaterial(this.ss6project.getTexture(cell.cellMap().name())), null, _pixi_constants.DRAW_MODES.TRIANGLES);
		}
		/**
		* メッシュセルからメッシュを作成
		...
		*/
		MakeMeshCellMesh(partID, cellID) {
			...
			if (uvLength > 0) {
				...
				for (let idx = 2; idx < uvLength; idx++) uvs[idx - 2] = meshsDataUV.uv(idx);
				...
				return new _pixi_mesh.Mesh(new _pixi_mesh.MeshGeometry(new Float32Array(meshNum * 2), uvs, indices), new _pixi_mesh.MeshMaterial(this.ss6project.getTexture(this.playerLib.fbObj.cells(cellID).cellMap().name())), null, _pixi_constants.DRAW_MODES.TRIANGLES);
			}
			return null;
		}
		/**
	...
	//#endregion
	//#region src/PluginParameters.js
	var PluginParameters = class PluginParameters {
		static instance;
		pluginParameters;
		animationDir;
		replaceSVActorSpriteFlag;
		svActorDir;
		svActorAnimationPack;
		svActorHideWeaponGraphics;
		replaceSVEnemySpriteFlag;
		svEnemyDir;
		svEnemyAnimationPack;
		...
		static getInstance() {
			if (PluginParameters.instance) return PluginParameters.instance;
			PluginParameters.instance = new PluginParameters();
			return PluginParameters.instance;
		}
	};
	//#endregion
	//#region src/SS6ProjectManager.js
	var SS6ProjectManager = class SS6ProjectManager {
		static instance;
		ssprojectMap;
		_isLoading;
		constructor() {
		...
		clear() {
			this.ssprojectMap = /* @__PURE__ */ new Map();
		}
		static getInstance() {
			if (SS6ProjectManager.instance) return SS6ProjectManager.instance;
			SS6ProjectManager.instance = new SS6ProjectManager();
			return SS6ProjectManager.instance;
		}
	};
	//#endregion
	//#region src/ss6player-rpgmakermz.js
	...
	let g_ss6playerPlayWaitingStatus = false;
	let g_passSS6PlayerToSpritePicture = null;
	let g_pictureSS6PlayerPrependCallback = null;
	let g_pictureSS6PlayerAppendCallback = null;
	PluginManager.registerCommand(PLUGIN_NAME, "loadSsfb", function(args) {
		...
		if (SS6ProjectManager.getInstance().isExist(ssfbId)) {
			if (ssfbPath === SS6ProjectManager.getInstance().get(ssfbId).ssfbPath) return;
		}
		this.setWaitMode(SS6PROJECT_LOAD_WAIT_MODE);
		SS6ProjectManager.getInstance().prepare(ssfbId);
		new SS6Project(ssfbPath, (proj, error) => {
			if (error) {
				this.setWaitMode("");
				console.error(error);
				throw error;
	...
	PluginManager.registerCommand(PLUGIN_NAME, "waitForPicture", function(args) {
		...
		if (picture && picture.mzkpSS6Player) {
			if (picture.mzkpSS6Player instanceof SS6Player) if (picture.mzkpSS6Player.loop !== -1) {
				...
				g_pictureSS6PlayerPrependCallback = function(ss6player) {
					if (ss6player.loop === 0) g_ss6playerPlayWaitingStatus = false;
				};
			} else console.warn("pictureId: " + pictureId + " can not wait SS6Player because setting infinity loop.");
		} else console.warn("pictureId: " + pictureId + " not have SS6Player");
	});
	const _Game_Interpreter_updateWaitMode = Game_Interpreter.prototype.updateWaitMode;
	Game_Interpreter.prototype.updateWaitMode = function() {
		let waiting = false;
		if (this._waitMode === SS6PROJECT_LOAD_WAIT_MODE) waiting = SS6ProjectManager.getInstance().isLoading();
		else if (this._waitMode === SS6PLAYER_WAIT_MODE) waiting = g_ss6playerPlayWaitingStatus;
		else waiting = _Game_Interpreter_updateWaitMode.call(this);
		return waiting;
	};
	...
	Game_Screen.prototype.erasePicture = function(pictureId) {
		...
		if (picture && picture.mzkpSS6Player) picture.mzkpSS6Player = null;
	...
	Sprite_Picture.prototype.updateBitmap = function() {
		...
		if (this.visible && this._pictureName === "") {
			...
			if (this.mzkpSS6Player !== player || playerChanged) if (player !== null) {
				if (player.loop === 0) {
				...
				player.SetPlayEndCallback(() => {
					if (prependCallback !== null) prependCallback(player);
					if (player.loop === 0) {
						player.Stop();
						spritePicture.removeChild(player);
					}
					if (appendCallback !== null) appendCallback(player);
				});
				if (this.mzkpSS6Player) {
					this.mzkpSS6Player.Stop();
					this.removeChild(this.mzkpSS6Player);
	...
	SceneManager.updateScene = function() {
		...
		if (this._scene) if (this.isGameActive()) {
			if (g_suspendPlayingSS6Player) {
				$gameScreen._pictures.forEach((picture, index, pictures) => {
					if (picture && picture.mzkpSS6Player) picture.mzkpSS6Player.Resume();
				});
				$gameActors._data.forEach((actor, index, actors) => {
					if (actor && actor._svActorSS6Player) actor._svActorSS6Player.Resume();
				});
				$gameTroop.members().forEach((enemy, index, enemies) => {
					if (enemy && enemy._svEnemySS6Player) enemy._svEnemySS6Player.Resume();
				});
				g_suspendPlayingSS6Player = false;
			}
		} else {
			if ($gameScreen && $gameScreen._pictures) $gameScreen._pictures.forEach((picture, index, pictures) => {
				if (picture && picture.mzkpSS6Player) picture.mzkpSS6Player.Pause();
			});
			if ($gameActors && $gameActors._data) $gameActors._data.forEach((actor, index, actors) => {
				if (actor && actor._svActorSS6Player) actor._svActorSS6Player.Pause();
			});
			if ($gameTroop && $gameTroop.members()) $gameTroop.members().forEach((enemy, index, enemies) => {
				if (enemy && enemy._svEnemySS6Player) enemy._svEnemySS6Player.Pause();
			});
	...
	DataManager.isDatabaseLoaded = function() {
		if (PluginParameters.getInstance().replaceSVEnemySpriteFlag) {
			...
			return true;
		} else return _DataManager_isDatabaseLoaded.call(this);
	};
	...
	Sprite_Actor.prototype.setBattler = function(battler) {
		...
		if (PluginParameters.getInstance().replaceSVActorSpriteFlag) {
			if (changed) {
				...
				if (notFoundSvActorSsfbMap.has(ssfbId)) return;
				if (SS6ProjectManager.getInstance().isExist(ssfbId)) {
	...
	Sprite_Actor.prototype.updateBitmap = function() {
		if (PluginParameters.getInstance().replaceSVActorSpriteFlag) {
			...
			if (SS6ProjectManager.getInstance().isExist(ssfbId) && this._actor._svActorSS6Player !== void 0) {
				Sprite_Battler.prototype.updateBitmap.call(this);
				this._mainSprite.bitmap = null;
				this.updateSS6Player();
			} else _Sprite_Actor_updateBitmap.call(this);
		} else _Sprite_Actor_updateBitmap.call(this);
	};
	Sprite_Actor.prototype.updateSS6Player = function() {
		...
		for (let key in Sprite_Actor.MOTIONS) {
			...
			if (this._motion === motion) motionName = key;
		}
		if (motionName === "") motionName = "walk";
		if (this._actor._svActorSS6Player === null || this._actor._svActorSS6Player.animeName !== motionName) {
			...
			this._actor._svActorSS6Player.SetPlayEndCallback((player) => {
				if (player.loop === 0) this.refreshMotion();
			});
	...
	Sprite_Actor.prototype.setupWeaponAnimation = function() {
		if (PluginParameters.getInstance().replaceSVActorSpriteFlag) if (PluginParameters.getInstance().svActorHideWeaponGraphics) {
			if (this._actor._svActorSS6Player === void 0) _Sprite_Actor_setupWeaponAnimation.call(this);
		} else _Sprite_Actor_setupWeaponAnimation.call(this);
		else _Sprite_Actor_setupWeaponAnimation.call(this);
	};
	...
	Sprite_Enemy.prototype.setBattler = function(battler) {
		...
		if (PluginParameters.getInstance().replaceSVEnemySpriteFlag) {
			if (changed) {
				...
				if (notFoundSvEnemySsfbMap.has(ssfbId)) return;
				let ssfbPath = Sprite_Enemy.svEnemySsfbPath(enemyId);
				const dataEnemy = this._enemy.enemy();
				if (dataEnemy._svEnemyFile) ssfbPath = PluginParameters.getInstance().animationDir + dataEnemy._svEnemyFile;
	...
	Sprite_Enemy.prototype.updateBitmap = function() {
		if (PluginParameters.getInstance().replaceSVEnemySpriteFlag) {
			const enemyId = this._enemy.enemyId();
			const ssfbId = Sprite_Enemy.svEnemySsfbId(enemyId);
			if (!this._enemy._svEnemySS6ProjectLoaded) return;
			if (SS6ProjectManager.getInstance().isExist(ssfbId) && this._enemy._svEnemySS6Player !== void 0) {
				Sprite_Battler.prototype.updateBitmap.call(this);
				this.bitmap = null;
				this.updateSS6Player();
			} else _Sprite_Enemy_updateBitmap.call(this);
		} else _Sprite_Enemy_updateBitmap.call(this);
	};
	Sprite_Enemy.prototype.updateSS6Player = function() {
		if (this._enemy._svEnemySS6Player === null) {
			...
			let offsetY = 0;
			const dataEnemy = this._enemy.enemy();
			if (dataEnemy._svEnemyAnimationPackName) animePackName = dataEnemy._svEnemyAnimationPackName;
			if (dataEnemy._svEnemyAnimationName) animeName = dataEnemy._svEnemyAnimationName;
			if (dataEnemy._svEnemyScaleX) scaleX = dataEnemy._svEnemyScaleX;
			if (dataEnemy._svEnemyScaleY) scaleY = dataEnemy._svEnemyScaleY;
			if (dataEnemy._svEnemyOffsetX) offsetX = dataEnemy._svEnemyOffsetX;
			if (dataEnemy._svEnemyOffsetY) offsetY = dataEnemy._svEnemyOffsetY;
	...
	Sprite_Enemy.prototype.updateFrame = function() {
		if (PluginParameters.getInstance().replaceSVEnemySpriteFlag) {
			...
			if (SS6ProjectManager.getInstance().isExist(ssfbId) && this._enemy._svEnemySS6Player !== void 0) {
				...
				if (this._effectType === "bossCollapse") this.setFrame(0, 0, width, this._effectDuration);
				else this.setFrame(0, 0, width, height);
			} else _Sprite_Enemy_updateFrame.call(this);
		} else _Sprite_Enemy_updateFrame.call(this);
	};
	const _Sprite_Enemy_updateStateSprite = Sprite_Enemy.prototype.updateStateSprite;
	Sprite_Enemy.prototype.updateStateSprite = function() {
		if (PluginParameters.getInstance().replaceSVEnemySpriteFlag) if (this.bitmap) {
			this._stateIconSprite.y = -Math.round((this.bitmap.height + 40) * .9);
			if (this._stateIconSprite.y < 20 - this.y) this._stateIconSprite.y = 20 - this.y;
		} else {
			let height = 0;
			if (this._enemy && this._enemy._svEnemySS6Player) height = this._enemy._svEnemySS6Player.height;
			this._stateIconSprite.y = -Math.round((height + 40) * .9);
			if (this._stateIconSprite.y < 20 - this.y) this._stateIconSprite.y = 20 - this.y;
		}
		else _Sprite_Enemy_updateStateSprite.call(this);
	};
	//#endregion
